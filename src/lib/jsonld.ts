...
// Two-type version of MultiThing. In the future, we could make this variadic.
export type MultiThing<
  T extends Thing = Thing,
  U extends Thing = Thing,
> = (Omit<T, "@type"> & Omit<U, "@type">) & {
  "@type": [Exclude<T, string>["@type"], Exclude<U, string>["@type"]];
};

export type WithMultiContext<T extends MultiThing> = T & {
...
export function JsonLd<T extends MultiThing>(
  item: WithMultiContext<T>,
  space?: string | number,
): string;
export function JsonLd(
  item: Graph | WithContext<Thing> | WithMultiContext<MultiThing>,
  space?: string | number,
): string {
